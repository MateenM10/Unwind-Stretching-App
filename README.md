...
The weighted shuffle wasn't actually weighted. The original sort comparator had an operator-precedence bug, `Math.random() - 0.5 / totalWeight` instead of `(Math.random() - 0.5) / totalWeight`, so favourited stretches were barely more likely to show up than anything else. I rewrote it using Efraimidis-Spirakis weighted sampling: give each stretch a key of `random^(1/weight)` and sort by that. It's a small algorithm, but getting it wrong the first time was a good reminder to actually verify behavior instead of just checking that the code compiles.
